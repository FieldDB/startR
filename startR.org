...
To recap:
    ...
    - The workspace is the R-internal set of "active" objects that R can manipulate and analyze.
...
** Creating data
There may be other times when you want to generate data within R itself, instead of reading everything from other files. For example, maybe you'd like to generate a series of normally-distributed random numbers (don't we all, from time to time?), or just a sequence of integers.  Here's some code that does these two things:
...
You'll recognize the assignment operator (the arrow) in all three. This means that in all three cases, we're not just generating a bunch of numbers, we're storing the values in an object.  In this example, =x= is a set of 1000 (pseudo-)random numbers taken from a normal distribution (that's what =rnorm= does), =y= is the sequence of integers from 1 to 1000 (that's what the =:= operator does), and =z= is a sequence of numbers from 0 to 1, in increments of 0.1 (that's one way to use the =seq= function). Now that we've created them, they're part of our workspace, alongside any other data that we've read in or created at this point.

This is one difference between working with R and other programs you might be used to: it's absolutely normal and typical for your workspace to have multiple datasets or other variables at once.  You aren't limited to just working with one "file" at a time.
